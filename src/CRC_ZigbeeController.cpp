...
** This will provide methods to scan/detect/configure
** a Zigbee module
**/
...
void ZigbeeController::init(HardwareSerial & serialPort, Messaging & wirelessComm)
{
...
boolean ZigbeeController::scanForModule()
{
	...
	{
		Logger.logF(Logger.LOG_INFO, F("XBEE:Checking for Device at Baud Rate: %lu"), BAUD_RATES[rateId]);
		_serialPort->flush();
		_serialPort->end();
		...
		if (enterCommandMode())
		{
			...
			Logger.logF(Logger.LOG_INFO, "XBEE:Found device at: %lu", _baudRate);
			Logger.log(Logger.LOG_INFO, F("XBEE:Identifying Device Type"));
			Logger.logF(Logger.LOG_INFO, F("XBEE:Device Type: %s"), sendCommand("DD", false));
...
boolean ZigbeeController::enterCommandMode()
{
	...
	Logger.log(Logger.LOG_TRACE, F("XBEE:Timeout Waiting for Command Mode"));
	return false;	
}

void ZigbeeController::exitCommandMode()
{
...
char* ZigbeeController::sendCommand(char* command, boolean atomic)
{
	if (atomic) {
		enterCommandMode();
	}
	...
	Logger.logF(Logger.LOG_TRACE, F("XBEE:Sending Command: %s"), command);
	_serialPort->print(F("AT"));
...
boolean ZigbeeController::isReady()
{
...
boolean ZigbeeController::isConnected(boolean logIsConnectedMessage, boolean atomic)
{
	if (strcmp_P(sendCommand("AI", atomic), (char *)F("0")) == 0)
	{
		...
		{
			Logger.logF(Logger.LOG_INFO, F("XBEE:Connected to Network '%s'"), getNetworkId(atomic));
...
boolean ZigbeeController::connectToNetwork()
{
	...
	if (_attemptedNetwork >= 10)
	{
		...
		Logger.log(Logger.LOG_INFO, F("XBEE:No configured Wifi Networks available"));
		return _isConnected;
	}
	...
	while(_attemptedNetwork < 10)
	{
		...
		if (ConfigurationManager.getConfig(temp, _receive, sizeof(_receive)))
		{
			countAttempted++;
			...
			// Configure SSID
			Logger.logF(Logger.LOG_INFO, F("XBEE:Setting SSID: %s"), _receive);
			...
			if (ConfigurationManager.getConfig(temp, _receive, sizeof(_receive)))
			{
				Logger.logF(Logger.LOG_INFO, F("XBEE:Setting Mode: %s"), _receive);
			...
			if (ConfigurationManager.getConfig(temp, _receive, sizeof(_receive)))
			{
				Logger.logF(Logger.LOG_INFO, F("XBEE:Setting PSK: %s"), _receive);
			...
			Logger.log(Logger.LOG_INFO, F("XBEE:Saving Wireless Configuration"));
	...
	if (countAttempted == 0 && firstAttempt)
	{
		...
		UnitState.wireless = WI_STATUS_WIFI_NOTAVAILABLE;
		Logger.log(Logger.LOG_INFO, F("XBEE:Wifi Networks not configured"));
		return false;
	}
...
*/
char * ZigbeeController::getNetworkId(boolean atomic)
{
